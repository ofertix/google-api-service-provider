        ...
        */
        $app['google.api.client'] = $app->share(function() use ($app) {
          ...
          if (!isset($app['google.api.service_account_name'])) {
            ...
            if (!isset($app['google.api.ofertix_token_file_location'])) {
                throw new \Exception("client_id api must be defined", 1);
            }

            if (!isset($app['google.api.beauty_token_file_location'])) {
                throw new \Exception("client_id api must be defined", 1);
            }

            if (!isset($app['google.api.nc_es_ofertix_token_file_location'])) {
                throw new \Exception("client_id api must be defined", 1);
            }

            if (!isset($app['google.api.nc_fr_ofertix_token_file_location'])) {
                throw new \Exception("client_id api must be defined", 1);
            }
